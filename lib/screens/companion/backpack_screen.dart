...
/// `CompanionScreen` already wraps every tab in — this widget wraps its
/// own content in `CompanionScreenContainer` for the standard tab inset
/// (like `MapScreen`/`SkillsScreen`; `AnimalsScreen` wraps itself in it
/// too, but with `hasPadding: false` — see that screen's own doc
/// comment).
/// This widget owns the `connection.state == null` guard (both need a
/// non-null `GameState` to render) and the one `LayoutBuilder` that
/// resolves the shared grid layout — see
